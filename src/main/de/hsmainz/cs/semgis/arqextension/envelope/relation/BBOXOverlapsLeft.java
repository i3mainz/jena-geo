	...
	public NodeValue exec(NodeValue v1, NodeValue v2) {
        ...
        try {
            ...
            try {
    			...
    			if(raster.getGridGeometry().getEnvelope().getUpperCorner().getCoordinate()[0]<
    					raster2.getGridGeometry().getEnvelope().getLowerCorner().getCoordinate()[0]) {
    						return NodeValue.TRUE;
    			}
    			return NodeValue.FALSE;
    		...
    		if(wrapper1 instanceof CoverageWrapper) {
    			GridCoverage2D raster=((CoverageWrapper)wrapper1).getGridGeometry();
    			GeometryWrapper geom2 = GeometryWrapper.extract(v2);
    			if(LiteralUtils.toGeometry(raster.getEnvelope2D()).intersects(LiteralUtils.toGeometry(geom2.getEnvelope()))){
    				return NodeValue.TRUE;
    			}
				if(raster.getEnvelope().getUpperCorner().getCoordinate()[0]<geom2.getEnvelope().getMinX()) {
					return NodeValue.TRUE;
				}
				return NodeValue.FALSE;
    		}else {
