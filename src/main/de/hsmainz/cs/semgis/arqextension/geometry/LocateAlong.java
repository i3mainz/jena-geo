...
public class LocateAlong extends FunctionBase3 {
	...
	public NodeValue exec(NodeValue arg0, NodeValue arg1, NodeValue arg2) {
		...
		return GeometryWrapperFactory
				.createMultiPoint(Arrays.asList(mp.getCoordinates()), geom1.getSrsURI(), geom1.getGeometryDatatypeURI())
				.asNodeValue();
	}
...
// Ignore hole
				...
				result.addAll(computePoints(subGeom.getCoordinates(), segmentLengthFraction, offsetDistance));
			}
		}
		return geom.getFactory().createMultiPoint(result.toArray(new Coordinate[0]));
	}

	private static Set<Coordinate> computePoints(Coordinate[] coords, double segmentLengthFraction,
