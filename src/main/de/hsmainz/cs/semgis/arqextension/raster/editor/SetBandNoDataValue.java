...
public class SetBandNoDataValue extends FunctionBase {
	...
	public NodeValue exec(List<NodeValue> args) {
		if(args.size()==2) {
			...
			List<Double> dlist=DoubleStream.of(raster.getSampleDimension(1).getNoDataValues()).boxed().collect(Collectors.toList());
			...
			BigInteger bandnum=args.get(1).getInteger();
			Double nodataval=args.get(2).getDouble();
			List<Double> dlist=DoubleStream.of(raster.getSampleDimension(bandnum.intValue()).getNoDataValues()).boxed().collect(Collectors.toList());
			if(dlist.contains(nodataval)) {
				return wrapper.asNodeValue();
			}else {
				return null;
